...
var SocialEngine = class {
  constructor(profile) {
    ...
    this.personalFacts = /* @__PURE__ */ new Map();
    this.interactionHistory = [];
  }
  generateGreeting(timeOfDay, lastInteractionHours) {
    const greetings = {
    ...
    if (lastInteractionHours === void 0) {
  ...
  rememberPersonalFact(fact) {
    this.personalFacts.set(fact.subject, fact.detail);
  }
  recordInteraction(content, sentiment) {
  ...
  getInteractionPatterns() {
    const allText = this.interactionHistory.map((i) => i.content).join(" ").toLowerCase();
    ...
    const avgSentiment = this.interactionHistory.length > 0 ? sentimentSum / this.interactionHistory.length : 0;
    let avgTimeBetween = 0;
    if (this.interactionHistory.length > 1) {
      const times = this.interactionHistory.map((i) => i.timestamp.getTime()).sort();
    ...
    return {
      ...
      topics: Object.entries(frequency).sort((a, b) => b[1] - a[1]).slice(0, 3).map(([word]) => word),
      sentimentTrend: avgSentiment,
      timeBetweenInteractions: avgTimeBetween
    };
  }
  getRecentTopics(count = 3) {
...
var MemoryManager = class {
  ...
  async getRelevantContext(query) {
    ...
    try {
      ...
      const similar = results.documents[0].filter(
        (doc, i) => (
          // Only include results with reasonable similarity
          results.distances?.[0]?.[i] !== void 0 && results.distances[0][i] < 0.5
        )
      ).map((doc) => doc.trim());
      Logger.debug(`Found ${similar.length} similar documents`);
      return [.../* @__PURE__ */ new Set([...recent, ...similar])];
    } catch (error) {
  ...
  async scrapeAndStoreWebpage(url2) {
    try {
      ...
      const cleanedText = text.replace(/\s+/g, " ").trim().substring(0, 5e3);
      this.memory.push({
        type: "scraped",
        content: cleanedText,
        timestamp: /* @__PURE__ */ new Date(),
  ...
  async getMemoryStats() {
    return {
      ...
      lastStored: this.memory.length > 0 ? new Date(Math.max(...this.memory.map((m) => m.timestamp.getTime()))) : void 0,
      scrapedCount: this.memory.filter((m) => m.type === "scraped").length
    };
  }
  async getScrapedContent(url2) {
    const scrapes = this.memory.filter((m) => m.type === "scraped" && (!url2 || m.url === url2)).map((m) => ({
      url: m.url,
      content: m.content,
